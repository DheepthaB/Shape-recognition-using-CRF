        ...
        for x in range(0,2):
            for pi in range(0,N):
                if x==train_features['pixels'][m][pi] and s==int(train_features['shape'][m]):
                    ...
                    #print cond
            t=1
            while (emp_prob[m,2*s+x]-float(prob[m,2*s+x])-(math.exp(-2)*wts_fpis[0,2*s+x]))>1e-1:
                wts_fpis[0,2*s+x]=wts_fpis[0,2*s+x]+(emp_prob[m,2*s+x]-prob[m,2*s+x]-(math.exp(-2)*wts_fpis[0,2*s+x]))*(2.0/(2.0+t))
                cond=conditional(s,pixels,wts_fs,wts_fpis)
...
for m in range(0,M):
    for s in range(0,3):
        if float(s)==float(train_features['shape'][m]):
